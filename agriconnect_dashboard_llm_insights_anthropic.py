...
# ---- LLM Chart Summary Function for Anthropic ----
def llm_chart_summary_anthropic(chart_title, df, columns=None, api_key=None):
    try:
        key = api_key or st.secrets.get("ANTHROPIC_API_KEY")
        if not key:
            return "⚠️ Anthropic API key not found. Set ANTHROPIC_API_KEY in Streamlit secrets."
        client = anthropic.Anthropic(api_key=key)
        if columns:
            df = df[columns]
        sample_df = df.sample(min(30, len(df)), random_state=42) if len(df) > 30 else df
        data_sample = sample_df.to_dict(orient="records")
        prompt = (
            f"You are an expert data analyst for an agricultural dashboard. "
            f"Given this data sample and chart title '{chart_title}', "
            f"write a short, plain-language summary of the main insight(s) for ordinary users. "
        ...
        # Claude's response is a list of content blocks; take the first .text for main content
        return response.content[0].text
    except Exception as e:
        return f"⚠️ LLM insight unavailable: {e}"

def insight_card_llm(title, summary, color="#e3f2fd"):
    st.markdown(
        f"""
...
with tab1:
    ...
    try:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "Regional Hotspots Map (PHL risk and Credit Score by Region)", 
                grouped, 
                columns=["region", "phl_risk_score", "predicted_credit_score"])
        insight_card_llm("💡 LLM Insights: Regional Hotspots", insight, color="#ffe0b2")
    except Exception as e:
...
with tab2:
    ...
    with col_a:
        ...
        try:
            ...
            with st.spinner("Generating insight..."):
                insight = llm_chart_summary_anthropic(
                    "Interventions Adopted Pie Chart",
                    filtered_df,
                    columns=["interventions_adopted"])
            insight_card_llm("💡 LLM Insights: Interventions Adopted", insight)
        except Exception as e:
        ...
        try:
            ...
            with st.spinner("Generating insight..."):
                insight = llm_chart_summary_anthropic(
                    "Feature Importances (simulated)",
                    imp_df,
                    columns=["Feature", "Importance"])
            insight_card_llm("💡 LLM Insights: Feature Importance", insight, color="#e8f5e9")
        except Exception as e:
    ...
    with col_b:
        ...
        try:
            ...
            with st.spinner("Generating insight..."):
                insight = llm_chart_summary_anthropic(
                    "Credit Score and PHL Risk Over Months",
                    trend_df.reset_index(),
                    columns=["harvest_month", "phl_risk_score", "predicted_credit_score"])
            insight_card_llm("💡 LLM Insights: Time Trends", insight, color="#fff9c4")
        except Exception as e:
        ...
        try:
            if {'predicted_credit_score', 'phl_risk_score', 'region'}.issubset(filtered_df.columns):
                ...
                with st.spinner("Generating insight..."):
                    insight = llm_chart_summary_anthropic(
                        "Farmer Segmentation Scatterplot",
                        filtered_df, 
                        columns=["predicted_credit_score", "phl_risk_score", "region"])
                insight_card_llm("💡 LLM Insights: Farmer Segmentation", insight, color="#ede7f6")
            else:
...
with tab3:
        ...
        if selected_farmer and selected_farmer != "-- Select --":
            ...
            with st.spinner("Generating insight..."):
                insight = llm_chart_summary_anthropic(
                    ...
                    farmer_row)
            insight_card_llm("💡 LLM Insights: Farmer Profile", insight, color="#f8bbd0")
    except Exception as e:
    ...
    try:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "High-Risk and Low-Credit Farmers",
                anomalies)
        insight_card_llm("💡 LLM Insights: High-Risk / Low-Credit Farmers", insight, color="#ffcdd2")
    except Exception as e:
...
with tab4:
    ...
    if display_cols:
        ...
        insight_card_llm("💡 LLM Insights: Farmers Table", insight, color="#d1c4e9")
    else:
...
with tab5:
    ...
    with col1:
        ...
        if 'farmer_id' in filtered_df.columns:
            ...
            with st.spinner("Generating insight..."):
                insight = llm_chart_summary_anthropic(
                    "Top 10 Farmers by Credit Score",
                    top_farmers,
                    columns=["farmer_id", "region", "predicted_credit_score", "phl_risk_score"]
                )
            insight_card_llm("💡 LLM Insights: Top Farmers", insight, color="#b2dfdb")
        else:
    ...
    with col2:
        ...
        if 'farmer_id' in filtered_df.columns:
            ...
            with st.spinner("Generating insight..."):
                insight = llm_chart_summary_anthropic(
                    "Bottom 10 Farmers by Credit Score",
                    bottom_farmers,
                    columns=["farmer_id", "region", "predicted_credit_score", "phl_risk_score"]
                )
            insight_card_llm("💡 LLM Insights: Bottom Farmers", insight, color="#ffcdd2")
        else:
            st.info("Farmer IDs not available.")
...
with tab6:
    ...
    if "interventions_adopted" in filtered_df.columns:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                ...
                eff_df)
        insight_card_llm("💡 LLM Insights: Intervention Effectiveness", insight, color="#e0f2f1")
    else:
    ...
    if {"region", "interventions_adopted"}.issubset(filtered_df.columns):
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "Intervention Adoption by Region Table",
                crosstab.reset_index())
        insight_card_llm("💡 LLM Insights: Intervention Adoption by Region", insight, color="#f9fbe7")
    else:
    ...
    if len(num_cols) >= 2:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                ...
                corr.reset_index())
        insight_card_llm("💡 LLM Insights: Correlation Matrix", insight, color="#fce4ec")
    else:
...
with tab8:
    ...
    if "predicted_credit_score" in filtered_df.columns:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                ...
                filtered_df,
                columns=["predicted_credit_score"])
        insight_card_llm("💡 LLM Insights: Credit Score Distribution", insight, color="#e0f7fa")
    else:
    ...
    if "phl_risk_score" in filtered_df.columns:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "PHL Risk Score Distribution Histogram",
                filtered_df,
                columns=["phl_risk_score"])
        insight_card_llm("💡 LLM Insights: PHL Risk Score Distribution", insight, color="#fff9c4")
    else:
...
with tab9:
    ...
    if {"predicted_credit_score", "farm_size"}.issubset(filtered_df.columns):
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "Credit Score vs Farm Size Scatterplot",
                filtered_df,
                columns=["farm_size", "predicted_credit_score", "region"])
        insight_card_llm("💡 LLM Insights: Credit Score vs. Farm Size", insight, color="#f3e5f5")
    else:
    ...
    if {"crop_type", "phl_risk_score"}.issubset(filtered_df.columns):
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "PHL Risk by Crop Type Bar Chart",
                crop_df,
                columns=["crop_type", "phl_risk_score"])
        insight_card_llm("💡 LLM Insights: PHL Risk by Crop Type", insight, color="#ffccbc")
    else:
    ...
    if {"gender", "predicted_credit_score", "phl_risk_score"}.issubset(filtered_df.columns):
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "Credit Score and PHL Risk by Gender",
                gender_df)
        insight_card_llm("💡 LLM Insights: Gender Analytics", insight, color="#f8bbd0")
    else:
    ...
    if "harvest_month" in filtered_df.columns:
        ...
        with st.spinner("Generating insight..."):
            insight = llm_chart_summary_anthropic(
                "Farmer Count by Harvest Month",
                by_month.reset_index(),
                columns=["harvest_month", "farmer_id"])
        insight_card_llm("💡 LLM Insights: Farmer Count by Month", insight, color="#dcedc8")
    else:
        st.info("No harvest month data available.")

st.caption("✨ Built with Streamlit | Analytics, reports, and LLM-powered natural language insights for AgriConnect, powered by Anthropic Claude.")

st.markdown("""
<sub>
<b>Note:</b> LLM-powered insights require an Anthropic API key.<br>
Set in <code>.streamlit/secrets.toml</code> as <code>ANTHROPIC_API_KEY</code>.
</sub>
""", unsafe_allow_html=True)