...
with st.sidebar:
    ...
    st.image("https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=500&q=80", use_column_width=True)
    st.markdown("---")
    st.header("🔎 Filters")
    # Regions
    ...
    # Credit Score
    min_score, max_score = st.session_state.get("min_score", 0.0), st.session_state.get("max_score", 1.0)
    credit_range = st.slider("Credit Score", min_value=float(min_score), max_value=float(max_score), value=(float(min_score), float(max_score)))
    # Interventions
...
try:
    ...
    # Store for sidebar
    st.session_state["regions"] = df['region'].dropna().unique().tolist()
    st.session_state["min_score"] = float(df['predicted_credit_score'].min())
    st.session_state["max_score"] = float(df['predicted_credit_score'].max())
    st.session_state["interventions"] = df['interventions_adopted'].dropna().unique().tolist()
