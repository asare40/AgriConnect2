...
# ---- LLM Chart Summary Function ----
def llm_chart_summary(chart_title, df, columns=None, api_key=None):
    """Generate an LLM-based summary for a given chart and dataframe sample."""
    try:
        key = api_key or st.secrets.get("OPENAI_API_KEY")
        if not key:
            return "⚠️ OpenAI API key not found. Set OPENAI_API_KEY in Streamlit secrets."
        openai.api_key = key
        if columns:
            df = df[columns]
...
with tab7:
    ...
    # No LLM summary needed for download
...
st.caption("✨ Built with Streamlit | Analytics, reports, and LLM-powered natural language insights for AgriConnect.")

st.markdown("""
<sub>
<b>Note:</b> LLM-powered insights require an OpenAI API key.  
Set in <code>.streamlit/secrets.toml</code> as <code>OPENAI_API_KEY</code>.
</sub>
""", unsafe_allow_html=True)